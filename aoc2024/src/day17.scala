...
object day17 {
  ...
  def out(state: State): State = {
    val operand = getOperant(combo = true, state = state)
    val mod     = operand % 8
    state.copy(ip = state.ip + 2, output = mod +: state.output)
  }
  ...
  def getOperant(combo: Boolean, state: State): Int = {
    if (combo == true) {
